...
final class provider_test extends \advanced_testcase {
    ...
    public function test_is_request_allowed(): void {
        ...
        set_config('globalratelimit', 5, 'aiprovider_groq');
        set_config('enableuserratelimit', 1, 'aiprovider_groq');
        set_config('userratelimit', 3, 'aiprovider_groq');
    ...
    public function test_is_provider_configured(): void {
        ...
        $provider = new \aiprovider_groq\provider();
        $this->assertFalse($provider->is_provider_configured());

        // Partially configured values.
        set_config('apikey', '123', 'aiprovider_groq');
        set_config('orgid', '', 'aiprovider_groq');
        $provider = new \aiprovider_groq\provider();
        $this->assertFalse($provider->is_provider_configured());

        // Properly configured values.
        set_config('apikey', '123', 'aiprovider_groq');
        set_config('orgid', 'abc', 'aiprovider_groq');
        $provider = new \aiprovider_groq\provider();
