...
class provider extends \core_ai\provider {
    ...
    public function __construct() {
        ...
        $this->apikey = get_config('aiprovider_groq', 'apikey');
        // Get api org id from config.
        $this->orgid = get_config('aiprovider_groq', 'orgid');
        // Get global rate limit from config.
        $this->enableglobalratelimit = get_config('aiprovider_groq', 'enableglobalratelimit');
        $this->globalratelimit = get_config('aiprovider_groq', 'globalratelimit');
        // Get user rate limit from config.
        $this->enableuserratelimit = get_config('aiprovider_groq', 'enableuserratelimit');
        $this->userratelimit = get_config('aiprovider_groq', 'userratelimit');
    }
    ...
    public function get_action_settings(
        ...
        if ($actionname === 'generate_text' || $actionname === 'summarise_text') {
            ...
            $settings[] = new \admin_setting_configtext(
                "aiprovider_groq/action_{$actionname}_model",
                new \lang_string("action:{$actionname}:model", 'aiprovider_groq'),
                new \lang_string("action:{$actionname}:model_desc", 'aiprovider_groq'),
                'llama-3.1-8b-instant',
                PARAM_TEXT,
            );
            // Add API endpoint.
            $settings[] = new \admin_setting_configtext(
                "aiprovider_groq/action_{$actionname}_endpoint",
                new \lang_string("action:{$actionname}:endpoint", 'aiprovider_groq'),
                '',
            ...
            $settings[] = new \admin_setting_configtextarea(
                ...
                new \lang_string("action:{$actionname}:systeminstruction", 'aiprovider_groq'),
                new \lang_string("action:{$actionname}:systeminstruction_desc", 'aiprovider_groq'),
